...
Watch the repo default branch's own CI. If the check on `{{defaultBranch}}`'s tip is failing and no fix is already in flight, open a single tracking issue and dispatch `@kody run` to fix it.
...
2. **Ensure one tracking issue (dedup):** `ensure_issue({ key: "default-branch-ci-red-{{defaultBranch}}", title: "{{defaultBranch}} CI is red - Kody auto-fix", body: <below> })`.
...
The reused issue (`key: "default-branch-ci-red-{{defaultBranch}}"`) is the whole dedup mechanism. While it is open, `ensure_issue` returns `created: false` and the capability stops. Closing it allows a later tick to open a fresh one.
