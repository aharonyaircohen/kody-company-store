---
name: system-audit
description: Audit Kody capability, agent, executable, report, and runtime-state wiring.
---

# System Audit Skill

Use this skill when the `system-audit` executable runs from the matching capability.
...
2. **Missing executable reference:** A capability profile names an executable but `.kody/executables/<slug>/profile.json` does not exist.
...
- `gh repo view`
- `gh api` reads for `.kody/capabilities`, `.kody/agents`, `.kody/executables`, `reports/` in the configured Kody state repo, `.kody/context`, and history/state when available
...
- Do not edit capabilities, agent, executables, reports, context, labels, PRs, or state.
- Do not re-kick work.
