...
Run the `code-health` agentAction. Its skills own the architecture-audit and type-debt methods.
...
- Run the `code-health` agentAction.

## Restrictions

- Disabled until the operator enables this grouped watch.
- Read-only on the codebase.
- Do not run build, test, lint, or code edits directly from the agentResponsibility.
