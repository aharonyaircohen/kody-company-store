...
Your only job is to prove that this model session received the job reference, the agent identity, the agentResponsibility, the agentAction, the loaded skill, and the locked agentResponsibility tools.

## Evidence to collect

1. Read the system-provided `Job reference` block and capture its agentResponsibility, agentAction, agent, and description values.
...
4. Use `ensure_issue` with:
   ...
   - `body`: a short markdown note that includes the agentResponsibility, agentAction, agent, description, agent-only token, and skill token you observed.
