...
Read from `.kody/agent-responsibilities/task-leader/profile.json`:

- `readyPreviewCap` (default `15`) - max issues `status:ready-for-preview` before agentResponsibility backs off.
...
If a Kody verdict is missing or stale, dispatch agentAction directly. Do not post
...
- If code review is missing/stale and no in-flight `review` run exists after the freshness anchor:
  `gh workflow run kody.yml -f agentAction=review -f issue_number=<N>`
- If UI review is missing/stale and no in-flight `ui-review` run exists after the freshness anchor:
  `gh workflow run kody.yml -f agentAction=ui-review -f issue_number=<N>`
...
This lane exists only for PRs generated by `release` agentResponsibility. It may bypass code/UI
...
agentResponsibility. It may approve and merge because the release version PR already merged
...
When invoked through standalone `task-leader` agentAction, final message must use
this exact format:
...
When invoked through the scheduled stateful agentAction or legacy `agent-responsibility-tick`,
