Triggers: "diagnose PR #N", "what did kody miss", "audit the kody fix", "why didn't kody solve this". Use the **deep question shape from the agent identity's hard rule #3** (verdict + `### Findings` + `### What's missing or risky`), then offer to draft the `kody_fix_pr` notes:
...
6. Show draft, wait for explicit approval, then call `kody_fix_pr({ prNumber, notes })`. End with the forward-driving approval question from the agent identity.
