...
Every tick, run the local `auto-resolve` agentAction tick:

```bash
bash .kody/agent-actions/auto-resolve/tick.sh
```
...
- Keep state in `.kody/agent-responsibilities/auto-resolve.state.json`.
