#!/usr/bin/env bash
# Deterministic agentAction-local tick for auto-resolve.

set -euo pipefail

STATE_FILE=".kody/agent-responsibilities/auto-resolve.state.json"
...
echo "[auto-resolve] now=$NOW_ISO open_non_draft_conflicting=$COUNT dry_run=$DRY_RUN"
echo
...
for ((i = 0; i < COUNT; i++)); do
  ...
  if [ "$DRY_RUN" != "1" ]; then
    gh workflow run kody.yml -f agentAction=resolve -f issue_number="$pr" >/dev/null
  fi
  new_entry=$(echo "$effective" | jq -c --arg s "$head" \
    '{lastSha:$s, attempts:((.attempts // 0)+1), stuck:false}')
  NEW_PERPR=$(echo "$NEW_PERPR" | jq -c --arg k "$pr" --argjson v "$new_entry" '. + {($k):$v}')
  ACTIONS_TAKEN+=("dispatched resolve on #$pr")
