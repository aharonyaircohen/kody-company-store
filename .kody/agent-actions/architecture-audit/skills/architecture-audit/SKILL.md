...
Use this skill when the `architecture-audit` agentAction runs from the matching agentResponsibility.
...
Periodic **architecture-health sweep** of the codebase — boundaries and coupling, not line-level style. The job itself cannot run shell beyond `gh`, so it opens a tracking issue delegating the analysis to a Kody agentAction in CI and tracks the result.
