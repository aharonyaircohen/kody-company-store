...
function hasLocalCapability(slug) {
  return exists(cwd, `.kody/capabilities/${slug}/definition.json`);
}

function hasStoreCapability(slug) {
  return exists(storeRoot, `capabilities/${slug}/definition.json`);
}
...
function readCapabilityDefinition(slug) {
  return (
    readJsonIfExists(cwd, `.kody/capabilities/${slug}/definition.json`) ||
    readJsonIfExists(storeRoot, `capabilities/${slug}/definition.json`)
  );
}
  ...
  for (const slug of activeCapabilities) {
    if (hasLocalCapability(slug)) {
      ...
      row(
        ...
        "repo-local",
        `.kody/capabilities/${slug}/definition.json`,
        "consumer repo",
        alsoStore ? "confirm override is intentional" : "keep local or promote to Store",
      );
    } else if (hasStoreCapability(slug)) {
      row("capabilities", slug, "store", "healthy", `.kody/capabilities/${slug}/definition.json`, "Store", "none");
    } else {
...
function loopIntent(slug, template, loopSource, expectedCapability, expectedTarget) {
  const capability = expectedCapability ? readCapabilityDefinition(expectedCapability) : null;
  ...
  if (expectedCapability && capability && outcome) {
